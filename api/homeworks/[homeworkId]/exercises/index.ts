...
import { addManualHomeworkExercise } from "../../../../src/server/homework-generation-service";
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    response.status(201).json(await addManualHomeworkExercise(String(homeworkId), request.body?.type));
  } catch (error) {
