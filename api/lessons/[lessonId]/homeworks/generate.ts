...
import { generateHomeworkDraftForLesson } from "../../../../src/server/homework-generation-service";
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    response.status(201).json(
      await generateHomeworkDraftForLesson(String(lessonId), { model: request.body?.model }),
    );
  } catch (error) {
